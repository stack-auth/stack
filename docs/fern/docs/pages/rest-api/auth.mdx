...
Stack requires some headers to be sent on every endpoint:
...
| `X-Stack-Access-Token` | string | Optional. The access token for the current user. If not given, the user will be assumed to be logged out. |
