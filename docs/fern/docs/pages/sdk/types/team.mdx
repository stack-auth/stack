...
type Team = {
  ...
  listUsers(): Promise<TeamUser[]>;  //$stack-link-to:#teamuser
  useUsers(): TeamUser[];  //$stack-link-to:#teamuser
};
```
...
Calling `serverUser.getTeam(...)` and `serverUser.listTeams()` will return `ServerTeam` objects if the user is a [`ServerUser`](../objects/user.mdx#serveruser). Alternatively, you can call `stackServerApp.getTeam('team_id_123')` or `stackServerApp.listTeams()` to query all teams of the project.
...
<div className="small-codeblock-tabs"><CodeBlocks>
```typescript Listing server team members in React component
const users = team.useUsers();
users.forEach(user => {
  console.log(user.id, user.teamProfile.displayName);
});
