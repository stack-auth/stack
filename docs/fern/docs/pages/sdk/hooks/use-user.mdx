...
This standalone React hook is an alias for `useStackApp().useUser()`. It is simply a convenience that's faster to type because it's so common; it does not have any additional functionality.
