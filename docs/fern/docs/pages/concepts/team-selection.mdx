...
To facilitate team selection, Stack provides a component that looks like this (TODO: Add image/visual representation):
...
To combine the switcher with the deep link method, you can pass in `urlMap` and `openedTeam`. The `urlMap` is a function to generate a URL based on the team information, and `openedTeam` is the team that the user is currently working on. This lets you implement "deep link" + "most recent team":
