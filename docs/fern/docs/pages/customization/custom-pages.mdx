...
By default, `StackHandler` creates all pages you need. You can customize the [colors](./custom-colors.mdx) and [components](./custom-components.mdx) here. However, if you'd like full control of the layout and logic flow, you can always use our built-in components to build your own pages or use even star from strach using low-level functions. 
