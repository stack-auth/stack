...
## Retrieving the current user in Client Components
...
Sometimes, you want to retrieve the user only if they're signed in, and redirect to the sign-in page otherwise. In this case, simply pass `{ or: "redirect" }` in the options:
...
## Retrieving the current user in Server Components
