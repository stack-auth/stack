...
1. **Create an OAuth App**: On the provider's website, create an OAuth app and set the callback URL to the corresponding Stack callback URL. Copy the client ID and client secret.
   <Tabs>
     ...
     <Tab title="LinkedIn">
       [LinkedIn OAuth Setup Guide](https://learn.microsoft.com/en-us/linkedin/shared/authentication/client-credentials-flow?context=linkedin%2Fcontext&tabs=HTTPS1)
       Callback URL:
       `https://api.stack-auth.com/api/v1/auth/oauth/callback/linkedin`
     </Tab>
     <Tab title="Hubspot">
       [Hubspot OAuth Setup Guide](https://developers.hubspot.com/beta-docs/guides/apps/authentication/working-with-oauth#query-parameters)
       Callback URL:
       `https://api.stack-auth.com/api/v1/auth/oauth/callback/hubspot`
     </Tab>
     <Tab title="Salesforce">
       [Salesforce OAuth Setup Guide](https://help.salesforce.com/s/articleView?id=sf.connected_app_create_api_integration.htm&type=5)
       Callback URL:
       `https://api.stack-auth.com/api/v1/auth/oauth/callback/salesforce`
     </Tab>
     <Tab title="Bitbucket">
     ...
     <Tab title="Zoom">
       [Zoom OAuth Setup Guide](https://developers.zoom.us/docs/integrations/create)
       Callback URL:
       `https://api.stack-auth.com/api/v1/auth/oauth/callback/zoom`
     </Tab>
     <Tab title="Dropbox">
       [Dropbox OAuth Setup Guide](https://developers.dropbox.com/oauth-guide)
       Callback URL:
       `https://api.stack-auth.com/api/v1/auth/oauth/callback/dropbox`
     </Tab>
     <Tab title="Coinbase">
       [Coinbase OAuth Setup Guide](https://docs.cdp.coinbase.com/sign-in-with-coinbase/docs/sign-in-with-coinbase-reference/)
       Callback URL:
       `https://api.stack-auth.com/api/v1/auth/oauth/callback/coinbase`
     </Tab>
     <Tab title="Instagram">
       [Instagram OAuth Setup Guide](https://developers.facebook.com/docs/instagram-basic-display-api/reference/oauth-authorize/)
       Callback URL:
       `https://api.stack-auth.com/api/v1/auth/oauth/callback/instagram`
     </Tab>
     <Tab title="Netlify">
       [Netlify OAuth Setup Guide](https://docs.netlify.com/security/secure-access-to-sites/oauth-provider-tokens/)
       Callback URL:
       `https://api.stack-auth.com/api/v1/auth/oauth/callback/netlify`
     </Tab>
   </Tabs>
