...
function UserButtonInnerInner(props: UserButtonProps & { user: CurrentUser | null }) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="stack-scope outline-none">
        <div className="flex items-center gap-2">
          <UserAvatar user={user} />
          {user && props.showUserInfo && (
            <div className="flex flex-col justify-center">
              <Typography className={textClasses}>{user.displayName}</Typography>
              <Typography className={textClasses} variant="secondary" type="label">
                {user.primaryEmail}
              </Typography>
            </div>
          )}
      ...
      <DropdownMenuContent className="stack-scope">
        ...
        <DropdownMenuSeparator />
        {user && (
          <Item text="Account settings" onClick={async () => await app.redirectToAccountSettings()} icon={<CircleUser {...iconProps} />} />
        )}
        {!user && <Item text="Sign in" onClick={async () => await app.redirectToSignIn()} icon={<LogIn {...iconProps} />} />}
        {!user && <Item text="Sign up" onClick={async () => await app.redirectToSignUp()} icon={<UserPlus {...iconProps} />} />}
        {user && props.extraItems && props.extraItems.map((item, index) => <Item key={index} {...item} />)}
        {props.colorModeToggle && <Item text="Toggle theme" onClick={props.colorModeToggle} icon={<SunMoon {...iconProps} />} />}
        {user && <Item text="Sign out" onClick={() => user.signOut()} icon={<LogOut {...iconProps} />} />}
      </DropdownMenuContent>
    </DropdownMenu>
  );
}
