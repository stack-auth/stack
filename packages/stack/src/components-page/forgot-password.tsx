...
export function ForgotPasswordForm({ onSent }: { onSent?: () => void }) {
  ...
  return (
    ...
    >
      ...
      <Input id="email" type="email" {...register("email")} onChange={() => clearErrors("email")} />
...
export function ForgotPassword({ fullPage = false }: { fullPage?: boolean }) {
  ...
  return (
    <MaybeFullPage fullPage={fullPage}>
      <div className="stack-scope mb-6 text-center" style={{ width: "380px", padding: fullPage ? "1rem" : 0 }}>
        <Typography type="h2">Reset Your Password</Typography>
        <Typography>
          {"Don't need to reset? "}
          <StyledLink href={stackApp.urls["signUp"]}>Sign in</StyledLink>
        </Typography>
      </div>
