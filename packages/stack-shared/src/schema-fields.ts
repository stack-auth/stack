...
import { providerList } from "./interface/crud/oauth";
...
export const oauthIdSchema = yupString().oneOf(providerList).meta({ openapiField: { description: `OAuth provider ID, one of ${providerList.map(x => `\`${x}\``).join(', ')}`, exampleValue: 'google' } });
