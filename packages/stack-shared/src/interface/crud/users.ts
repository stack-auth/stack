  ...
  .yupObject({
    ...
    password: fieldSchema
      ...
      .meta({
        openapiField: {
          description:
            "A new password for the user, overwriting the old one (if it exists). Specifying this option revokes all current sessions.",
          exampleValue: "my-new-password",
        },
      }),
    totp_secret_base64: fieldSchema.base64Schema.nullable().meta({
      openapiField: {
...
export const usersCrudServerReadSchema = fieldSchema
  .yupObject({
     ...
     * @deprecated
     */
    requires_totp_mfa: fieldSchema
      .yupBoolean()
      .required()
      .meta({
        openapiField: { hidden: true, description: "Whether the user is required to use TOTP MFA to sign in", exampleValue: false },
      }),
    /**
     * @deprecated
     */
    oauth_providers: fieldSchema
      ...
      .meta({
        openapiField: {
          hidden: true,
          description: "A list of OAuth providers connected to this account",
          exampleValue: [{ id: "google", account_id: "12345", email: "john.doe@gmail.com" }],
        },
      }),
    auth_methods: fieldSchema
      .yupArray(
        fieldSchema.yupUnion(
          fieldSchema
            .yupObject({
      ...
      .meta({
        openapiField: {
          hidden: true,
          description: "A list of authentication methods available for this user to sign in with",
          exampleValue: [{ contact_channel: { email: "john.doe@gmail.com", type: "email" }, type: "otp" }],
        },
      }),
    connected_accounts: fieldSchema
      .yupArray(
        fieldSchema.yupUnion(
          fieldSchema
            .yupObject({
      ...
      .meta({
        openapiField: {
          hidden: true,
          description: "A list of connected accounts to this user",
          exampleValue: [{ provider: { provider_user_id: "12345", type: "google" }, type: "oauth" }],
        },
      }),
...
const webhookUserDeletedSchema = fieldSchema
  .yupObject({
    id: fieldSchema.userIdSchema.required(),
  })
  .required();
