...
export const usersCrudServerReadSchema = yup.object({
  ...
  has_password: yup.boolean().required().meta({ openapi: { description: 'Whether the user has a password', exampleValue: true } }),
  auth_with_email: yup.boolean().required().meta({ openapi: { description: 'Whether the user can authenticate with their primary e-mail. If set to true, the user can log-in with credentials and/or magic link, if enabled in the project settings.', exampleValue: true } }),
  oauth_providers: yup.array(yup.string().required()).required().meta({ openapi: { description: 'All the OAuth providers connected to this account', exampleValue: ['google', 'github'] } }),
