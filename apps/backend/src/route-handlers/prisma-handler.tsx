...
type WhereUnique<T extends AllPrismaModelNames> =
  Prisma.TypeMap["model"][Capitalize<T>]["operations"]["findUniqueOrThrow"]["args"]["where"];
...
type Include<T extends AllPrismaModelNames> = (Prisma.TypeMap["model"][Capitalize<T>]["operations"]["findMany"]["args"] & {
  include?: unknown;
})["include"];
type BaseFields<T extends AllPrismaModelNames> = Where<T> & Partial<PCreate<T>>;
type PRead<T extends AllPrismaModelNames, W extends Where<T>, I extends Include<T>> = GetResult<
...
type CrudToPrismaContext<AllParams extends boolean, PS extends ParamsSchema, QS extends QuerySchema> = Context<AllParams, PS, QS> & {
  type: "update" | "create";
};
type OnPrepareContext<AllParams extends boolean, PS extends ParamsSchema, QS extends QuerySchema> = Context<AllParams, PS, QS> & {
...
> = {
  ...
  transformPrismaToCrudObject(
    prismaOrNull: PRead<PrismaModelName, W & B, I> | null,
    params: yup.InferType<PS>,
    query: yup.InferType<QS>,
    context: Pick<Context<false, PS, QS>, "auth">,
  ): Promise<CRead<CrudTypeOf<S>>>;
};
...
>(
  ...
  options: {
    ...
    orderBy?: (
      context: Context<false, PS, QS>,
    ) => Promise<Prisma.TypeMap["model"][Capitalize<PrismaModelName>]["operations"]["findMany"]["args"]["orderBy"]>;
    include: (context: Context<false, PS, QS>) => Promise<I>;
    crudToPrisma: (
      crud: CEitherWrite<CrudTypeOf<S>>,
      context: CrudToPrismaContext<false, PS, QS>,
    ) => Promise<PEitherWrite<PrismaModelName>>;
  ...
  const wrapper = <AllParams extends boolean, T>(
    allParams: AllParams,
    func: (data: any, context: Context<AllParams, PS, QS>) => Promise<T>,
  ): ((opts: Context<AllParams, PS, QS> & { data?: unknown }) => Promise<T>) => {
    return async (req) => {
  ...
  return typedAssign(
    createCrudHandlers<any, PS, QS, any>(crudSchema, {
