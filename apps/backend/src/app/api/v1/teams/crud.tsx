...
export const teamsCrudHandlers = createLazyProxy(() =>
  createCrudHandlers(teamsCrud, {
    querySchema: yupObject({
      ...
      add_current_user: yupString()
        .oneOf(["true", "false"])
        .optional()
        .meta({
          openapiField: {
            onlyShowInOperations: ["Create"],
            description:
              "If to add the current user to the team. If this is not `true`, the newly created team will have no members. Notice that if you didn't specify `add_current_user=true` on the client side, the user cannot join the team again without re-adding them on the server side.",
            exampleValue: "true",
          },
        }),
    }),
      ...
      if (auth.type === "client") {
        ...
        if (userId !== currentUserId) {
          throw new StatusError(
            ...
            'Client can only list teams for their own user. user_id must be either "me" or the ID of the current user',
          );
        }
      }
