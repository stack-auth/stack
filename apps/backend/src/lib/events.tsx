...
function stripEventTypeSuffixFromKeys<T extends Record<`${string}EventType`, unknown>>(
  t: T,
): { [K in keyof T as K extends `${infer Key}EventType` ? Key : never]: T[K] } {
...
type DataOf<T extends EventType> = yup.InferType<T["dataSchema"]> & DataOfMany<T["inherits"]>;
...
export async function logEvent<T extends EventType[]>(
  ...
  const timeRange =
    "start" in timeOrTimeRange && "end" in timeOrTimeRange ? timeOrTimeRange : { start: timeOrTimeRange, end: timeOrTimeRange };
    ...
    try {
      ...
      if (error instanceof yup.ValidationError) {
        throw new StackAssertionError(
          `Invalid event data for event type: ${eventType.id}`,
          { eventType, data, error, originalData, originalEventTypes: eventTypes },
          { cause: error },
        );
      }
      throw error;
    ...
    for (const eventType of allEventTypes) {
      ...
      posthog.capture({
        ...
        groups: filterUndefined({
          projectId:
            typeof data === "object" && data && "projectId" in data
              ? typeof data.projectId === "string"
                ? data.projectId
                : throwErr("Project ID is not a string for some reason?", { data })
              : undefined,
        }),
