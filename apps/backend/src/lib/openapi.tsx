    ...
    paths: Object.fromEntries(
      [...options.endpoints]
        .map(([path, handlersByMethod]) => [
          ...
          Object.fromEntries(
            [...handlersByMethod]
              .map(([method, handler]) => [method.toLowerCase(), parseRouteHandler({ handler, method, path, audience: options.audience })])
              .filter(([, handler]) => handler !== undefined),
          ),
        ])
        .filter(([, handlersByMethod]) => Object.keys(handlersByMethod).length > 0)
        .sort(([, handlersByMethodA], [, handlersByMethodB]) =>
          ((Object.values(handlersByMethodA)[0] as any).tags[0] ?? "").localeCompare(
            (Object.values(handlersByMethodB)[0] as any).tags[0] ?? "",
          ),
        ),
    ),
  };
...
export function parseWebhookOpenAPI(options: { webhooks: readonly WebhookEvent<any>[] }) {
  return {
    openapi: "3.1.0",
    info: {
...
function isSchemaArrayDescription(
  value: yup.SchemaFieldDescription,
): value is yup.SchemaInnerTypeDescription & { type: "array"; innerType: yup.SchemaInnerTypeDescription } {
  return value.type === "array";
}

function isSchemaTupleDescription(
  value: yup.SchemaFieldDescription,
): value is yup.SchemaInnerTypeDescription & { type: "tuple"; innerType: yup.SchemaInnerTypeDescription[] } {
...
function parseRouteHandler(options: {
  ...
  for (const overload of options.handler.overloads.values()) {
    ...
    result = parseOverload({
      ...
      responseTypeDesc:
        undefinedIfMixed(responseDescribe.fields.bodyType) ??
        throwErr("Response type must be defined and not mixed", { options, bodyTypeField: responseDescribe.fields.bodyType }),
      statusCodeDesc:
        undefinedIfMixed(responseDescribe.fields.statusCode) ??
        throwErr("Status code must be defined and not mixed", { options, statusCodeField: responseDescribe.fields.statusCode }),
    });
  }
...
function getFieldSchema(
  field: yup.SchemaFieldDescription,
  crudOperation?: Capitalize<CrudlOperation>,
): { type: string; items?: any; properties?: any; required?: any } | undefined {
  ...
  switch (field.type) {
    ...
    case "object": {
      return {
        type: "object",
        properties: typedFromEntries(
          typedEntries((field as any).fields).map(([key, field]) => [key, getFieldSchema(field, crudOperation)]),
        ),
  ...
  if (isSchemaObjectDescription(description)) {
    return {
      ...
      properties: Object.fromEntries(
        Object.entries(description.fields).map(([key, field]) => {
