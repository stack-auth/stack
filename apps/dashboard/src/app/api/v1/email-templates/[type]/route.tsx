...
const crudHandlers = createCrudHandlers(emailTemplateCrud, {
  ...
  async onUpdate({ auth, data, params }) {
    ...
    const oldEmailTemplate = await getEmailTemplate(auth.project.id, typedToUppercase(type));
    if (oldEmailTemplate) {
      return await updateEmailTemplate(auth.project.id, typedToUppercase(type), data);
    } else {
      return await createEmailTemplate(auth.project.id, typedToUppercase(type), data);
    }
  },
  async onDelete({ auth, params }) {
    const type = await typeSchema.validate(params.type);
    const emailTemplate = await getEmailTemplate(auth.project.id, typedToUppercase(type));
    if (!emailTemplate) {
      throw new StatusError(StatusError.NotFound, 'Email template not found');
    }
    await deleteEmailTemplate(auth.project.id, typedToUppercase(type));
  },
});
