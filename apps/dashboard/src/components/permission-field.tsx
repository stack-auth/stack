...
export class PermissionGraph {
  ...
  updatePermission(permissionId: string, permission: AdminTeamPermissionDefinition) {
...
export function PermissionListField<F extends FieldValues>(
  props: {
    control: Control<F>;
    name: Path<F>;
      ...
      render={({ field }) => {
        return (
          <FormItem>
            ...
            <div className="max-h-64 flex-col overflow-y-auto rounded-lg border p-3 shadow-sm">
              {[...graph.permissions.values()].map((permission) => {
                ...
                const ancestors = graph
                  .recursiveAncestors(permission.id)
                  .map((p) => p.id)
                  .filter(
                    (id) =>
                      id !== permission.id &&
                      id !== CURRENTLY_EDITED_PERMISSION_SENTINEL &&
                      currentPermission.containedPermissionIds.includes(id),
                  );
                const inheritedFrom = contain && ancestors.length > 0 && `(from ${ancestors.join(", ")})`;
                return (
                  <label className="-my-3 flex flex-row justify-start gap-2 py-3" key={permission.id}>
                    <FormControl>
                      <Checkbox
                        checked={selected}
                        onCheckedChange={(checked) => {
                          ...
                          setGraph(
                            graph.updatePermission(CURRENTLY_EDITED_PERMISSION_SENTINEL, {
                              ...currentPermission,
                    ...
                    <FieldLabel>
                      {permission.id}
                      {inheritedFrom && <span className="ml-1 text-gray-500">{inheritedFrom}</span>}
                    </FieldLabel>
                    <FormMessage />
                  </label>
                );
